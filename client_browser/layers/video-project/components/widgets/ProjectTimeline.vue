  ...
  <div class="flex">
    <section class="relative">
      <div class="flex justify-between items-center w-[400px]">
        <BaseHeading
          size="md"
          class="mx-4 my-2 text-gray-600 dark:bg-muted-700"
        >
          {{ title }}
        </BaseHeading>
      ...
      <!-- <BaseButton @click="isReadyForPreview = !isReadyForPreview">{{
        !isReadyForPreview ? "Preview" : "Stop"
      }}</BaseButton> -->
    </section>

    <div class="w-1/2 bg-red-200" v-if="isReadyForPreview">
      <video-player
        :src="
          'http://localhost:8080/stream/grouped-segments?' +
